...
#####↓nとm逆！！！要注意！！！　cの第一列、第一行目は全要素0で固定なので、数はn+1,m+1なことに要注意！！！
    ...
    ###↓行優先ループ！！！
########↓外側の0を含めないと、大体の結果が正答の-1となってしまう！！！外側の0大事！！！
    #for i in range(1,m):
    #    for j in range(1,n):
    ...
    #           これは番兵みたいなもの。表の左上から矢印を右下に下ろしていくことで考える。その起点は
    #           考慮されないので、外に番兵置いておいて、表の全ての値が考慮されるようにしないといけない！！！
    for i in range(m):
       for j in range(n):
        ######↓三種の漸化式に基づいて、場合分けしてcを更新していく！！！
            if x[i]==y[j]:
...
######↓一つ前のループの行要素リスト
    ...
    #####↓TLE!!!
        ...
        #elif l[j+1] < m:
        #    l[j+1] = m
    #####↓↓↓↓↓↓↓↓max(l[j],l_pre[j+1])考慮せずに、左の数字と比べるだけでも良い？？？
        if x == y:
            l[j+1] = l_pre[j] + 1
        elif l[j+1] < l[j]:
            l[j+1] = l[j]
